...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        hf = last_hidden[0]
        hb = last_hidden[1]
        hcat = torch.cat([hf, hb], dim=1)
        init_decoder_hidden = self.h_projection(hcat)

        cf = last_cell[0]
        cb = last_cell[1]
        ccat = torch.cat([cf, cb], dim=1)
        init_decoder_cell = self.c_projection(ccat)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = enc_hiddens_proj.bmm(dec_hidden.unsqueeze(dim=2))
